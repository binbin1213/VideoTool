...
摘要（关键默认值）：
- 字体：正文 14pt/20lh（`type.md`），按钮/强调 16pt（`type.lg`），标题 20–24pt；
- 颜色：文本（浅）#111827，（深）#F3F4F6；边框（浅）#E5E7EB，（深）#2A2F3A；主色 #FF6A00；
- 控件高：按钮/输入框 32pt，圆角 8；列表行高 32pt；
- 图标：20/24px；
- 间距：8pt 网格（4/8/12/16/24/32）；
- 动效：180ms 标准缓动；
- 状态：hover/pressed 叠加透明度 0.04/0.08；禁用不透明度 0.38；
- 网格：窗口左右 16pt 内边距，分区间距 24pt，最大内容宽 1240。

组件样式规范：`docs/HandBrake_UI_组件样式规范.md`（含 Button/Input/Tabs/List/Toolbar/Progress/StatusBar/Alert）。
